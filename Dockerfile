...
RUN echo 'APT::Get::Assume-Yes "true";' > /etc/apt/apt.conf.d/borells \
  && echo 'DPkg::Options "--force-confnew";' >> /etc/apt/apt.conf.d/borells
...
RUN groupadd --gid 3434 borells \
  ...
  && echo 'borells ALL=NOPASSWD: ALL' >> /etc/sudoers.d/50-borells \
  && echo 'Defaults    env_keep += "DEBIAN_FRONTEND"' >> /etc/sudoers.d/env_keep

USER borells
ENV PATH /home/borells/.local/bin:/home/borells/bin:${PATH}
